...
class YouTubeLoaderError(PdfLoaderError):
    pass

class YouTubeLoader(PdfLoader):
    async def run(
        self,
        url: str,
        ...
        try:
            # extract video ID
            parsed = urlparse(url)
            vid = parse_qs(parsed.query).get("v", [parsed.path.split("/")[-1]])[0]
            ...
            raw = api.fetch(vid, preserve_formatting=True).to_raw_data()
        except Exception as e:
            raise YouTubeLoaderError(f"failed to fetch transcript: {e}")

        if not raw:
            raise YouTubeLoaderError(f"No transcript available for video ID {vid}")
